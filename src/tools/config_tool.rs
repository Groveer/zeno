//! Config tool — read current rcode configuration.
...
impl Tool for ConfigTool {
    ...
    fn schema(&self) -> Value {
        json!({
            ...
            "function": {
                "name": "config",
                "description": "Read or display rcode configuration.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "action": {
...
/// Mask API keys in config output.
fn mask_api_keys(content: &str) -> String {
    let lines: Vec<String> = content
        .lines()
        .map(|line| {
            if line.contains("api_key:") && !line.contains("api_key_env:") {
