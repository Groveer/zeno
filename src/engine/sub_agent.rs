...
///
/// `task_id` is a UUID generated by `record_spawn_edge()` — this is intentionally
/// different from the main agent's session ID. Sub-agents are ephemeral, in-process
/// work units that don't need cross-process isolation. The UUID scopes their graph
/// store edges within the parent session's topology.
